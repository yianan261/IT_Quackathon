...
class ModelService:
    ...
    async def retrieve_relevant_documents(self,
        ...
        if not self.search_available:
            logger.warning("RAG search is not available, skipping document retrieval")
            return []
    ...
    def _should_use_rag(self, query: str) -> bool:
        ...
        for pattern in function_call_indicators:
            if re.search(pattern, query, re.IGNORECASE):
                logger.info(f"Query likely needs function call, disabling RAG: {query}")
                return False
                
        # Course comparison pattern - this needs RAG
        course_comparison = re.search(r'(?:compare|difference|between)\s+(?:cs|CS)\s*(\d+)\s+(?:and|vs|versus|to|with)\s+(?:cs|CS)\s*(\d+)', query, re.IGNORECASE)
        ...
        for pattern in informational_indicators:
            if re.search(pattern, query, re.IGNORECASE):
                logger.info(f"Informational query detected, enabling RAG: {query}")
                return True
    ...
    def generate_response(self,
        ...
        try:
            ...
            if function_result:
                ...
                logger.info("Skipping RAG in generate_response as it's not async-compatible")
