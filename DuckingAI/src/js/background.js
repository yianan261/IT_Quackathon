...
async function pollForAutomation() {
  ...
  if (automationState.isRunning || automationState.processingLock) {
    console.log(
      "Automation already running or processing lock active, skipping poll"
    );
    return;
  }
  ...
  try {
    ...
    if (
      currentUrl.includes("login.stevens.edu/app/UserHome") ||
    ...
    if (now - automationState.lastRunTime < 30000) {
      console.log(
        `Last automation was ${
          (now - automationState.lastRunTime) / 1000
        }s ago, skipping poll`
      );
      return;
    }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    switch (message.type) {
      ...
      case "TOGGLE_POLLING":
        ...
        sendResponse({
          status: "polling_" + (message.enabled ? "started" : "stopped"),
        });
        break;
      case "WORKDAY_CARD_CLICKED":
        ...
        if (!automationState.navigationInProgress) {
          ...
          // 获取或使用默认的Workday URL
          const workdayUrl =
            message.href || "https://wd5.myworkday.com/stevens/";
          ...
          chrome.storage.local.get("pendingAutomation", (data) => {
            if (data.pendingAutomation) {
              ...
              setTimeout(() => {
                handleCurrentStep(
                  sender.tab.id,
                  data.pendingAutomation.instruction
                ).catch((error) =>
                  console.error(
                    "Error resuming automation after content script load:",
                    error
                  )
                );
              }, 2000);
...
async function triggerAutomation() {
  ...
  if (pendingData.pendingAutomation) {
    console.log(
      "There is a pending automation task, resuming instead of starting new"
    );
    ...
    if (tabs && tabs.length > 0) {
      ...
      await handleCurrentStep(
        tabs[0].id,
        pendingData.pendingAutomation.instruction
      ).catch((error) =>
        console.error("Error resuming pending automation:", error)
      );
    }
  ...
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.BASE_URL}${API_CONFIG.AUTOMATION_ENDPOINT}`
    );
...
async function executeAutomation(instruction) {
  // 支持多种自动化动作类型
  if (
    instruction.action !== "navigate_and_click" &&
    instruction.action !== "multi_step_navigation"
  ) {
  ...
  try {
    ...
    // 检查当前URL以避免重复导航
    if (
      tab.url === instruction.target_url ||
      (automationState.lastNavigationUrl === instruction.target_url &&
        Date.now() - automationState.lastRunTime < 10000)
    ) {
      console.log(
        "Already at target URL or recently navigated there, skipping navigation"
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      function listener(tabId, changeInfo, updatedTab) {
        if (tabId === tab.id && changeInfo.status === "complete") {
          ...
          setTimeout(() => {
            // 获取当前步骤
            handleCurrentStep(tab.id, instruction)
              .then(resolve)
              .catch((error) => {
...
async function handleCurrentStep(tabId, instruction) {
  try {
    ...
    if (instruction.action === "navigate_and_click") {
      ...
      console.log(
        `Processing step ${currentStepIndex} of multi-step navigation`
      );
      console.log(
        `Current step description: ${
          instruction.steps[currentStepIndex]?.description || "unknown"
        }`
      );
      console.log(
        `Current step action: ${
          instruction.steps[currentStepIndex]?.action || "unknown"
        }`
      );
      ...
      if (executedSteps.includes(currentStepIndex)) {
        console.log(
          `Step ${currentStepIndex} already executed, skipping to next step`
        );
        ...
        if (nextStepIndex >= instruction.steps.length) {
          console.log(
            "All steps already completed, clearing pending automation"
          );
      ...
      if (currentStepIndex < instruction.steps.length) {
        ...
        const isAcademicsStep =
          currentStep.description &&
          currentStep.description.includes("Academics");
        if (isAcademicsStep) {
          console.log(
            "Special processing for Academics step - ensuring proper navigation flow"
          );
        }
            ...
            chrome.tabs.sendMessage(
              ...
              async (response) => {
                if (chrome.runtime.lastError) {
                  console.error(
                    "Error sending verify dialog message:",
                    chrome.runtime.lastError
                  );
                  reject(chrome.runtime.lastError);
                } else {
                  console.log(
                    `Successfully verified dialog in step ${currentStepIndex}`
                  );
        ...
        if (!isLastStep) {
          ...
          console.log(
            "This is the last step, will clear pendingAutomation after execution"
          );
        }
      } else {
    ...
    return new Promise((resolve, reject) => {
      console.log(
        `Sending AUTOMATION_CLICK message to tab ${tabId} for element: ${
          elementsToClick?.[0]?.description || "unknown"
        }`
      );
      chrome.tabs.sendMessage(
        tabId,
        ...
        async (response) => {
          if (chrome.runtime.lastError) {
            ...
            // 给重要步骤额外的处理时间
            const isImportantNavigation =
              elementsToClick?.[0]?.description?.includes("Academics");
            if (isImportantNavigation) {
              console.log(
                "Important navigation step completed, waiting a bit before proceeding..."
              );
  ...
  if (changeInfo.status === "complete") {
    chrome.storage.local.get("pendingAutomation", async (data) => {
      ...
      if (pendingAutomation && pendingAutomation.instruction) {
        ...
        setTimeout(() => {
          handleCurrentStep(tabId, pendingAutomation.instruction).catch(
            (error) => {
